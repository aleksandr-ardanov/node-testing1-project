...
describe('[Exercise 6] Car', () => {
  ...
  test('[16] driving the car uses gas', () => {
    expect(focus.drive(1000)).toEqual(600)
  })
  test('[17] refueling allows to keep driving', () => {
  focus.drive(300)
    expect(focus.refuel(10)).toEqual(600)
  focus.drive(1)
  expect(focus.refuel(23)).toEqual(600)
  })
  test('[18] adding fuel to a full tank has no effect', () => {
  expect(focus.refuel(900)).toEqual(600)
  expect(focus.refuel(1)).toEqual(600)
  })
})
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  ...
  test('[21] rejects an error with the message "number must be a number" if passed a non-number type',async() =>{
    const result = await utils.isEvenNumberAsync('foo')
    expect(result).toMatch('number must be a number')
  })
  test('[22] rejects an error with the message "number must be a number" if passed NaN',async() =>{
    ...
    expect(result).toMatch(/number must be a number/i)
  })
})
